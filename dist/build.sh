...
do
    ...
    CGO_ENABLED=1 HOST=i686-w64-mingw32 CC=i686-w64-mingw32-gcc CXX=i686-w64-mingw32-g++ GOOS=windows GOARCH=386 go build -ldflags "-s -w -H=windowsgui -extldflags=-static" -o bin/"$BINNAME"_windows_386.exe $addon && ok
    echo -n -e "|- bin/$BINNAME""_windows_amd64.exe "
    CGO_ENABLED=1 HOST=x86_64-w64-mingw32 CC=x86_64-w64-mingw32-gcc CXX=x86_64-w64-mingw32-g++ GOOS=windows GOARCH=amd64 go build -ldflags "-s -w -H=windowsgui -extldflags=-static" -o bin/"$BINNAME"_windows_amd64.exe $addon && ok
